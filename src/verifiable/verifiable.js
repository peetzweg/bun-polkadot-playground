...
export function one_shot(entropy, members, context, message) {
    try {
        ...
        wasm.one_shot(retptr, addHeapObject(entropy), addHeapObject(members), addHeapObject(context), addHeapObject(message));
...
* @returns {Uint8Array}
*/
export function validate(proof, members, context, message) {
    const ret = wasm.validate(addHeapObject(proof), addHeapObject(members), addHeapObject(context), addHeapObject(message));
...
* @returns {Uint8Array}
*/
export function sign(entropy, message) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        wasm.sign(retptr, addHeapObject(entropy), addHeapObject(message));
        var r0 = getInt32Memory0()[retptr / 4 + 0];
        var r1 = getInt32Memory0()[retptr / 4 + 1];
        var r2 = getInt32Memory0()[retptr / 4 + 2];
        if (r2) {
...
* @returns {boolean}
*/
export function verify_signature(signature, message, member) {
    const ret = wasm.verify_signature(addHeapObject(signature), addHeapObject(message), addHeapObject(member));
    return takeObject(ret);
}
...
* @returns {Uint8Array}
*/
export function member_from_entropy(entropy) {
    const ret = wasm.member_from_entropy(addHeapObject(entropy));
    return takeObject(ret);
}
...
async function __wbg_init(input) {
    ...
    if (typeof input === 'undefined') {
        input = new URL('verifiable_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
